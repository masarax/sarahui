...
const allowed = new Set('id class name title type value placeholder required disabled checked min max step autocomplete rows role tabindex href download target rel selected hidden open multiple pattern minlength maxlength form inputmode readonly autofocus spellcheck contenteditable dir lang'.split(' '));
...
export function Dialog({id,title='Create a project',description='Give your next idea a place to grow.',children='',footer=''}={}) {
  id=uid('dialog',id);return raw('<dialog'+attributes({id,class:'s-dialog','aria-modal':'true','aria-labelledby':id+'-title','aria-describedby':description?id+'-description':null})+'><header class="s-dialog-header"><div><h2 id="'+escapeText(id)+'-title">'+escapeText(title)+'</h2>'+(description?'<p id="'+escapeText(id)+'-description">'+escapeText(description)+'</p>':'')+'</div>'+render(IconButton({icon:'close',label:'Close dialog',variant:'ghost',attrs:{'data-s-dialog-close':true}}))+'</header><div class="s-dialog-body">'+render(children)+'</div>'+(footer?'<footer class="s-dialog-footer">'+render(footer)+'</footer>':'')+'</dialog>');
}
...
export function CommandPalette({id='s-command',title='Search the library',items=[]}={}) {
  return html`<dialog id="${id}" class="s-dialog s-command" data-s-command aria-label="${title}"><div class="s-command-search">${Icon({name:'search',size:22})}<input type="search" role="combobox" aria-label="${title}" aria-expanded="false" aria-autocomplete="list" aria-controls="${id}-results" data-s-command-input data-s-autofocus placeholder="What are you looking for?" autocomplete="off">${IconButton({icon:'close',label:'Close search',variant:'ghost',attrs:{'data-s-dialog-close':true}})}</div><div class="s-command-list" id="${id}-results" role="listbox" aria-label="Results">${items.map((item,i)=>html`<div role="option" aria-selected="false" id="${id}-option-${i}" data-href="${safeHref(item.href)}" data-keywords="${item.keywords||item.label}">${Icon({name:item.icon||'layers',size:19})}<span>${item.label}</span>${Icon({name:'arrow',size:15})}</div>`)}</div><p data-s-command-empty hidden>No results. Try a different search.</p><footer><span><kbd>↑</kbd> <kbd>↓</kbd> to navigate</span><span><kbd>↵</kbd> to open</span><span><kbd>esc</kbd> to close</span></footer></dialog>`;
}
